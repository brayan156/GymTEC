...
create or alter   procedure generar_planilla @idsucursal int as begin
        ...
        (case when Empleado.idPlanilla=1 then 1
              when Empleado.idPlanilla=2 then SUM(IIF(DATEDIFF(month, fecha, getdate()) = 0 and fecha <= getdate() and
                                                      (C2.horaFin <= datepart(hour, getdate()) or fecha!=getdate() ),
        ...
        from Empleado left join Clase C2 on Empleado.cedula = C2.idEmpleado
...
create or alter procedure filtro_clases @id_sucursal int,@nombre_servicio varchar(50), @fechainicio date, @fechafin  date,@id_cliente int as begin
    ...
    where (S2.id=@id_sucursal or @id_sucursal=0) and (S3.nombre=@nombre_servicio or @nombre_servicio ='') and (Clase.fecha>=@fechainicio or @fechainicio = '' or @fechainicio is null) and (Clase.fecha<=@fechafin or @fechafin ='' or @fechafin is null) and Clase.fecha>=getdate() and (@id_cliente!=CC.cedulaCliente or cedulaCliente is null)
...
create or alter procedure tratamientos_gimnasio @id_sucursal int as begin
    select Tratamiento.id, Tratamiento.nombre,Tratamiento.imagen, iif(Ts.idSucursal is not null or Ts.idSucursal!=0,'asociado','no asociado') as disponibilidad from Tratamiento
    cross join Sucursal
...
GO
 ...
 create or alter procedure productos_gimnasio @id_sucursal int as begin
    select Producto.codigoBarras, Producto.nombre, Producto.imagen, costo, descripcion, iif(PS.idSucursal is not null or PS.idSucursal!=0,'asociado','no asociado') as disponibilidad from Producto
    cross join Sucursal
...
GO
 ...
 create or alter procedure inventario_gimnasio @id_sucursal int as begin
    select nSerie, marca, Equipo.descripcion, Equipo.imagen, TE.nombre as tipo, iif(Equipo.idSucursal is not null or Equipo.idSucursal!=0,'asociado','no asociado') as disponibilidad from Equipo
...
GO
 ...
 create or alter procedure filtro_clase_cliente @id_cliente int as begin
    ...
    join Sucursal S2 on E.idSucursal = S2.id
    join Servicio S3 on Clase.idServicio = S3.id
    where   Clase.fecha>=getdate() and (Clase.horaInicio>DATEPART(HOUR, GETDATE()) or Clase.fecha!=getdate()) and ClaseCliente.cedulaCliente=@id_cliente
 end;
GO
